/**
 * Attendance calendar day + punch times for the ops UI.
 *
 * Default: wall clock is interpreted in **Asia/Kolkata** (IST, no DST), so
 * "10:30 AM" saves as the correct UTC instant even if the PC/browser is set to UTC.
 *
 * Override: set `VITE_ATTENDANCE_TIMEZONE=local` (or `browser`) to use the machine's local zone.
 * Any other value is an IANA name, e.g. `America/New_York`.
 */
...
/**
 * ECMA `getTimezoneOffset` style value for `dateStr`'s midnight in the attendance zone:
 * minutes to add to `Date.UTC(y,m-1,d,0,0,0,0)` to get that zone's midnight (same as web_backend).
 */
...
/**
 * Wall clock on the given **attendance-zone** calendar day → UTC epoch ms.
...
/**
 * Fields to send with every attendance write/read so the API can rebuild the day window.
 */
...
/** Format a stored UTC instant as clock time in the attendance zone (or browser if local). */
...
/** Modal / form defaults from a stored UTC instant → 12h parts in attendance zone. */
